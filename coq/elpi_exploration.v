...
This is mostly for my own benefit, but you may gain something by
following along and watching me solve some simple problems using Elpi.

A good place to start with Elpi is the tutorials:

- `Tutorial on the Elpi programming language <https://lpcic.github.io/coq-elpi/tutorial_elpi_lang.html>`_
- `Tutorial on higher-order abstract syntax for Coq terms <https://lpcic.github.io/coq-elpi/tutorial_coq_elpi_HOAS.html>`_
- `Tutorial on Coq commands <https://lpcic.github.io/coq-elpi/tutorial_coq_elpi_command.html>`_
- `Tutorial on Coq tactics <https://lpcic.github.io/coq-elpi/tutorial_coq_elpi_tactic.html>`_
    
In order to use Elpi to write tactics, you will need to constantly
reference the Coq/Elpi API to see how to accomplish certain things.
The functionality available is spread out over a mix of Elpi files in
the Coq-Elpi Github repository. Two important ones are

- `coq-builtin.elpi <https://github.com/LPCIC/coq-elpi/blob/2ef66b2640af1e3ef2e859f7b49b40d47272e10a/builtin-doc/coq-builtin.elpi>`_
- `coq-HOAS.elpi <https://github.com/LPCIC/coq-elpi/blob/2ef66b2640af1e3ef2e859f7b49b40d47272e10a/elpi/coq-HOAS.elpi>`_

You may need to consult other files in the same coq-elpi/elpi
directory to find definitions.

Let's first understand the way Elpi models the domain. A [goal] in
Elpi is documented `here <https://github.com/LPCIC/coq-elpi/blob/2ef66b2640af1e3ef2e859f7b49b40d47272e10a/builtin-doc/coq-builtin.elpi>`_

From glancing through the codebase, it seems like an idiom is that,
for "algebraic data types" with one constructor, you give the
constructor the same name as the type (a kind of punning). In
particular the type [goal] has a single constructor, also called
[goal].

This goal wraps the following arguments: - the goal context - the
"trigger" argument - the goal formula (the type we are trying to
construct an inhabitant of) - the current proof, which is allowed to
depend on the context hypotheses - the list of arguments passed to the
parent tactic

We put the "algebraic data types" in quotes because I think there is
no exhaustiveness checking for constructors. |*)
...
(*| Our first example will match the goal to a product type, and, if
it is a product we will refine it using an application. The Coq
:math:`\forall` binder is represented in Elpi using the [prod] constant,
`here <https://github.com/LPCIC/coq-elpi/blob/2ef66b2640af1e3ef2e859f7b49b40d47272e10a/builtin-doc/coq-builtin.elpi>`_
    
The term [forall (x: A), B] would be represented as something like
[prod `x` {{ A }} (y\ {{ B(y) }})]
    
where the dependence of [B] on [x] is captured by making it a
function.
...
Goal forall x y : ⟪$\mathbb{N}$⟫, x = x.
Proof.
    elpi intro1.
    elpi intro1.
    (*| Hypotheses: [X, elpi_ctx_entry_1 : nat] |*)
    Abort.
...
Elpi Accumulate lp:{{
    solve (goal Ctx _ GoalType _ [trm Eq] as G) GL :-
      ...
      pi x\ (copy J x :- 
        ...
        copy GoalType (P x),
        if (occurs x (P x))
             (refine (match (app [Eq, Hole]) (fun _ S (a\ fun _
              {{ @eq lp:S lp:{{ Q0 Hole}} lp:{{Q Hole}} }}
              (_\ P a))) [{{_}}]) G GL )
...
(*| An interesting bug I hit while working through this next one: Elpi
usually parses the name of a global reference as a global reference.
But if you have implicit arguments enabled, and automatic application
turned on, then Coq will automatically apply your term to
metavariables before feeding it to Elpi, and once it's applied, it's a
term, not a global reference. |*)

Elpi Tactic rewrite3.
(*| Argument either the name of a hypothesis in the context, or the
name of a global definition (variables count as global references as
far as Elpi is concerned, although you can get a list of the
section-local variables by calling coq.env.section.
...
Elpi Accumulate lp:{{
    ...
    nested_forall Eqpf {{@eq lp:S lp:P lp:Q }}
      (goal _ _ GoalType _ _ as G) GL :- 
...
Variable sym : forall {a : A} {b : B a} (c c' : C a b),
    add c c' = add c' c.
